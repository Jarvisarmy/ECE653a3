  ...
  ensures pivot < u ==> partitioned(a, pivot, pivot, pivot+1, u);
{
  ...
  while (j < u)
    decreases u-j;
    invariant pv == a[u];
    //invariant forall x,y :: l <= x <= i && i < y < j ==> a[x] < a[y];
    invariant l-1 <= i < j <= u;
    // all [l,i] are <= pv
    invariant forall x :: l <= x <= i ==> a[x] <= pv;
    // all (i,j) are > pv
    invariant forall x :: i < x < j ==> a[x] > pv;

    // never touch [0,l-1]
    invariant l > 0 ==> beq(old(a[..]), a[..], 0, l-1);
    invariant l > 0 ==> partitioned(a, 0, l-1, l, u);

    // never touch [u+1, a.Length)
...

  {
    ...
    {
      i := i + 1;
      // swap the one lower than pv to [l,i]
      var t := a[i];
      a[i] := a[j];
      a[j] := t;
    }
